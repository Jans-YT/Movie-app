...
Alasan: Arsitektur ini memastikan Separation of Concerns. Logika bisnis tidak bocor ke dalam build(), dan 
pengujian dapat dilakukan pada lapisan Data tanpa perlu merender UI.
...
Karena aplikasi menggunakan data JSON lokal (array statis), pagination disimulasikan menggunakan List.skip().take() dan Future.delayed. Di skenario produksi dengan REST API nyata, ini akan diganti dengan parameter page dan limit di URL request.

Penyimpanan Shared Preferences

Untuk memenuhi syarat zero config tanpa setup tambahan, data favorite disimpan menggunakan SharedPreferences (berupa List of String IDs). Pada aplikasi berskala lebih besar, penyimpanan relasional lokal seperti SQLite/Drift akan lebih direkomendasikan daripada Key Value store.
...
Tool yang digunakan: Google Gemini dan Claude.
